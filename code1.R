  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      checkboxGroupInput(inputId = "resultados", label = "Resultados deseados",
                         choices = c("Resumen numérico HDS"="rhds", "Resumen numérico nuevos datos"="rnd", 
                                     "MYT", "Murphy", "DFT"), selected = NULL)
    ...
    for(j in alarma){
      ...
      local({
        ...
        output[[obsname]] <- renderUI({
          tags$p("Para la observación", my_i)
      ...
      local({
        ...
        output[[obsname]] <- renderUI({
          tags$p("Para la observación ",my_i,
                 ...
                 tags$br(),
                 "Nivel de confianza simultáneo ",Ksim)
...
#Textos para saber a que función corresponden las salidas
  output$t1 <- renderUI({
    ...
    if("MYT" %in% input$resultados){
      tags$p(tags$b("Método MYT"), align = "center")
    }
    })
  ...
  output$t2 <- renderUI({
    ...
    if("Murphy" %in% input$resultados){
      tags$p(tags$b("Método de Murphy"), align = "center")
    }
  })
  ...
  output$t3 <- renderUI({
    ...
    if("DFT" %in% input$resultados){
      tags$p(tags$b("Método DFT"), align = "center")
    }
  })
  ...
  output$rhds <- renderUI({
    ...
    if("rhds" %in% input$resultados){
      tags$p(tags$b("Resumen numérico del HDS"), align = "center")
    }
  }, )
  ...
  output$rnd <- renderUI({
    ...
    if("rnd" %in% input$resultados){
      tags$p(tags$b("Resumen numérico de los nuevos datos"), align = "center")
    }
  }, )
  ...
  output$thds <- renderUI({
    ...
    if("rhds" %in% input$resultados){
      ...
      colnames(t2hds)<-c("Observación", expression(T^2))
  ...
  output$tnd <- renderUI({
    ...
    if("rnd" %in% input$resultados){
      ...
      colnames(t2nd)<-c("Observación", expression(T^2))
      t2nd <- as.matrix(t2nd)
      ...
      # Señales
      señales <- matrix(reso$ResultadosND$Señales)
      colnames(señales) <- c("Señales")
      output$signs <- renderTable({señales},bordered = TRUE,  spacing="xs",
                                  width = 'auto', align = 'c',  
                                  rownames = TRUE)
